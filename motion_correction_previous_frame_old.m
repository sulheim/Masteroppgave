...
%Perform motion correction using previous(1,2,3??) frame(s) for reference
   ...
   for i = frame_indexes(4:end)
       ...
       curr_BG = imresize(log_compress(curr_RF), [512, 512], 'bicubic');
       ...
       if mse_before(i) >= 0.001
           [aligned_BG, optimization_data] = align_image(prev_BG, curr_BG, type, 200); 
       else
           aligned_BG = curr_BG;
           optimization_data = 0;
       end
   ...
   corrected_data.mse_after = mse_after_prev;
